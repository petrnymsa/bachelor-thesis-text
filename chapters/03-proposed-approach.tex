...
Gantt chart is widely used in time-management projects and systems. It is well known, established and used. It was found, that Gantt chart offers the best approach, how tasks can be visualised. 
...
Transaction box is associated rectangle with transaction which indicates current state of transaction. The state is where the transaction within \textit{Transaction Pattern} currently is. Current state also indicates progress value which is displayed as inner rectangle inside transaction box filled with colour. Progress value is computed as percentage value from current state. The state \textit{Requested} corresponds to 10 \%, \textit{Promised} to 25 \%, \textit{Executed} to 50 \%, \textit{Stated} to 75 \% and so on.  
...
The transaction boxes are placed accordingly to \gls{ocd} model. This means:
\begin{itemize}
...
\begin{itemize}
\item Response link is displayed as straight line with arrow at the end. On one side it has circle with \textit{Source C-Act} abbreviation, on the other side with arrow it has square with \textit{Target C-Act} abbreviation.
\item Waiting links is displayed as response link, but line between is dashed. 
\end{itemize}

Links are placed between appropriate transactions accordingly to \textit{source} and \textit{target} C-Acts, just like at \gls{psd}. However, if \textit{response} link has \textit{target} C-Act as ``Request'', the abbreviation of \textit{target} is omitted and link itself is displayed little differently \todo{img state-to-request}. It is totally clear, that this type of response means exactly ``If given C-Act at  source transaction occurred then target transaction is also requested''.
