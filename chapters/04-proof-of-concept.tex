...
The Transaction Box Control derives from \mintinline{C}{SKCanvasView}, which comes from SkiaSharp. The class provides method \mintinline{C}{void OnPaintSurface(SKPaintSurfaceEventArgs e)} which is called everytime when the graphics must be refreshed. \mintinline{C}{SKPaintSurfaceEventArgs} object provides \mintinline{C}{SKCanvas} object. \mintinline{C}{SKCanvas} contains methods for basic 2D drawing such as \mintinline{C}{DrawLine}, \mintinline{C}{DrawCircle} or for example \mintinline{C}{DrawPath} which can be used to draw any defined path, method support also SVG syntax. 
% ---------------------------------------------------------------------------------
